...
def trainer(args):
    ...
    for epoch in range(params.epochs):
        ...
        for phase in ['train']:
            ...
            for i, XB in enumerate(tqdm(dset_loaders[phase])):
                ...
                # BTF = torch.tensor(BTF).to(torch.long)
                ...
                # print('d_real',d_real.size(), 'real_labels',real_labels.size())
                d_real_loss = criterion_D(d_real, real_labels)
                # print('d_fake',d_fake.size(), 'fake_labels',fake_labels.size())
                ...
                # print('d_real_acu',d_real_acu.size())
                # print('d_fake_acu',d_fake_acu.size())
                ...
                # print (X)
                fake = G(Z, BTF)  # generated fake: 0-1, X: 0/1
                d_fake = D(fake, BTF)
                # print('d_fake',d_fake.size(), 'real_labels',real_labels.size())
                adv_g_loss = criterion_D(d_fake, real_labels)

                # recon_g_loss = criterion_D(fake, X)
                # print('fake',fake.size(), 'X',X.size())
            ...
            if (epoch + 1) % params.model_save_step == 0:
                ...
                # print('all-fake',fake.cpu().data.squeeze().numpy().shape)
                condition = BTF.cpu().data[:1].squeeze().numpy()
            ...
            G_losses.append(epoch_loss_adv_G)
            D_losses.append(epoch_loss_D)

    # print(cons)
    np.savetxt("conditions.csv", cons, delimiter=",")
